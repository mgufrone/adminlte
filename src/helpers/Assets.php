    ...
    */
    public function renderJs($position=self::ON_HEAD)
    {
        ...
        array_walk($js, function($value) use (&$result){
            $result .= HTML::script($value);
        });
        $custom = $this->getCustomJs($position);
        if(!empty($custom))
        {
            $customJs = '<script type="text/javascript" id="assets-adminlte-js-'.substr(md5(time()),0,5).'">';
            if(array() !== $custom)
    ...
    */
    public function renderCss()
    {
        ...
        array_walk($css, function($value) use (&$result){
            $result .= HTML::style($value);
        });
        $custom = $this->getCustomCss();
        if(!empty($custom))
        {
            $customCss = '<style type="text/css" rel="stylesheet" id="assets-adminlte-css-'.substr(md5(time()),0,5).'">';
            if($custom !== array())
    ...
    */
    public function registerJs($filePath="",$position=self::ON_HEAD, $package_name='')
    {
        if(!empty($filePath))
        {
            ...
            $this->js[$position][$package_name.$filePath] = $filePath;
        }
        // print_r($this->js);
        return $this;
    }
    ...
    */
    public function registerCss($filePath="",$package_name='')
    {
        if(!empty($filePath))
        {
            if(!empty($package_name))
                $package_name .= ':';
            $this->css[$package_name.$filePath] = $filePath;
        }
        return $this;
    }
